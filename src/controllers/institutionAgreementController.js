...
/**
 * Tüm kurum anlaşmalarını getirir
 */
async function getInstitutionAgreements(req, res, next) {
  try {
    ...
    // Her anlaşma için kategori indirimlerini al; JSON için sayısal alanların sayı olduğundan emin ol
    const agreements = await Promise.all(
      result.rows.map(async (row) => {
        ...
        const category_discounts = categoryDiscountsResult.rows.reduce(
          (acc, r) => {
...
/**
 * Yeni kurum anlaşması oluşturur
 */
async function createInstitutionAgreement(req, res, next) {
  try {
    ...
    if (discountPercentage < 0 || discountPercentage > 100) {
      return next(
        new AppError('Discount percentage must be between 0 and 100', 400),
      );
    }
    ...
    // Sağlanmışsa kategori indirimlerini ekle
    if (categoryDiscounts && typeof categoryDiscounts === 'object') {
      try {
        for (const [categoryName, discountPercent] of Object.entries(
          categoryDiscounts,
        )) {
        ...
        logger.error(
          { err: categoryErr, agreementId: agreement.id, categoryDiscounts },
          'Failed to insert category discounts',
        );
        // Kategori indirimleri başarısız olsa bile devam et - anlaşma oluşturuldu
      }
    }

    // Yanıt için kategori indirimlerini getir
    const categoryDiscountsResult = await query(
      'SELECT category_name, discount_percentage FROM institution_agreement_category_discounts WHERE institution_agreement_id = $1',
      [agreement.id],
    );
    const categoryDiscountsObj = categoryDiscountsResult.rows.reduce(
      (acc, row) => {
    ...
    await logDataEvent({
      ...
      changes: {
        institutionName,
        discountPercentage,
        categoryDiscounts: categoryDiscountsObj,
      },
    });
    ...
    logger.error(
      { err, body: req.body },
      'Failed to create institution agreement',
    );
    return next(
      new AppError(
        `Failed to create institution agreement: ${err.message}`,
        500,
      ),
    );
  }
...
async function updateInstitutionAgreement(req, res, next) {
  try {
    ...
    // Sağlanmışsa kategori indirimleri güncellemesini işle
    if (
      updates.categoryDiscounts &&
      typeof updates.categoryDiscounts === 'object'
    ) {
      // Mevcut kategori indirimlerini sil
      await query(
        'DELETE FROM institution_agreement_category_discounts WHERE institution_agreement_id = $1',
        [agreementId],
      );

      // Yeni kategori indirimlerini ekle
      for (const [categoryName, discountPercent] of Object.entries(
        updates.categoryDiscounts,
      )) {
...
/**
 * Kurum anlaşmasını mantıksal olarak siler (soft delete)
 */
async function deleteInstitutionAgreement(req, res, next) {
  try {
    ...
    return res
      .status(200)
      .json({ message: 'Institution agreement deleted successfully' });
  } catch (err) {
...
/**
 * Tüm indirim nedenlerini getirir
 */
