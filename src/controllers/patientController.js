...
const { notifyRole } = require('../services/notificationHub');
...
async function createPatient(req, res, next) {
  try {
    ...
    // Sağlanmışsa indirim nedenlerini ilişkilendir
    if (
      discountReasonIds &&
      Array.isArray(discountReasonIds) &&
      discountReasonIds.length > 0
    ) {
    ...
    await notifyRole('secretary', {
      type: 'patient',
      title: 'Yeni Hasta',
      message: `${firstName} ${lastName} kaydedildi`,
      data: { patientId: patient.id, action: 'created' },
    });
...
/**
 * Tüm hastaları sayfalama ve arama ile getirir
 */
async function getPatients(req, res, next) {
  try {
    ...
    // Ad, e-posta veya telefona göre ara
    if (search) {
      conditions.push(
...
/**
 * ID'ye göre tek bir hasta getirir
 */
...
async function updatePatient(req, res, next) {
  try {
    ...
    // Sağlanmışsa indirim nedenlerini güncelle
    if (updates.discountReasonIds !== undefined) {
      ...
      if (
        Array.isArray(updates.discountReasonIds) &&
        updates.discountReasonIds.length > 0
      ) {
...
/**
 * Hastayı siler (mantıksal silme / soft delete)
 */
